...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (contents[index].priority() < contents[parentIndex(index)].myPriority) {
    ...
    private void sink(int index) {
        ...
        if (leftIndex(index) <= size()) {
            validIndex = leftIndex(index);
        } else {
            return;
        }
        if (rightIndex(index) <= size() && contents[leftIndex(index)].priority() > contents[rightIndex(index)].priority()) {
            validIndex = rightIndex(index);
        }
        if (contents[validIndex].priority() < contents[index].priority()) {
            swap(validIndex, index);
            sink(validIndex);
        }
    }
